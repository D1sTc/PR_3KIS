...
namespace PR_3
{
    public partial class Form1 : Form
    {
        ...
        private void Multiply_Click(object sender, EventArgs e)
        {
            ...
            if(result == 1) textBoxNumbersExScreen.Text = "А что перемножать? :("; // Выводим результат умножения в текстовое поле
            else textBoxNumbersExScreen.Text = result.ToString(); // Выводим результат умножения в текстовое поле

            // MessageBox.Show("Результат умножения: " + result); // Выводим результат в сообщении, довольно интересная тема на будущие работы >:З
        ...
        private void textBoxNumbersKeys_TextChanged(object sender, EventArgs e) // Ошибочки №194
        {
            try
            {
                ...
                if (!int.TryParse(textBoxNumbersKeys.Text, out inputNumbers)) errorProviderSymbol.SetError(Generation, "Ошибка. Пожалуйста, введите целочисленное значение.");
            ...
            {
                MessageBox.Show("Нет данных"); // Выводим результат в сообщении, довольно интересная тема на будущие работы >:З
            }
        }
        ...
        private void Cubing_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Dictionary<int, int> randomKeys = new Dictionary<int, int>(); // Создаем новый словарь для хранения ключей и их кубов
            ...
            {
                MessageBox.Show("Нет данных. Введите или сгенерируйте ключи для возведения в куб"); // Выводим результат в сообщении, довольно интересная тема на будущие работы >:З
            }
        }
